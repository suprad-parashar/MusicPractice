...
import { TANPURA_PATTERNS, type TanpuraPatternId, type Octave } from '@/lib/tanpuraTone';
...
interface TanpuraSidebarProps {
  ...
  onPatternChange: (v: TanpuraPatternId) => void;
}
...
export default function TanpuraSidebar({
  ...
  const startTanpura = async () => {
    if (isPlayingRef.current) return;
    try {
      tanpuraTone.setTanpuraPattern(pattern); // Set pattern before starting
  ...
  const handlePatternChange = (newPattern: TanpuraPatternId) => {
    onPatternChange(newPattern);
    tanpuraTone.setTanpuraPattern(newPattern); // Immediate response
  };
